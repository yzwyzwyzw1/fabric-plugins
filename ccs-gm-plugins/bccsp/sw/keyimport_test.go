...
import (
	...
	mocks2 "github.com/Hyperledger-TWGC/fabric-plugins/ccs-gm-plugins/bccsp/mocks"
	"github.com/Hyperledger-TWGC/fabric-plugins/ccs-gm-plugins/bccsp/sw/mocks"
...
func TestSM2PKIXPublicKeyImportOptsKeyImporter(t *testing.T) {
	...
	raw,err:=gmX509.MarshalPKISM2PublicKey(&k.PublicKey)
	require.NoError(t, err)
...
func TestSM2KeyGenAndImport(t *testing.T){
	...
	raw,err:=gmX509.MarshalPKISM2PrivateKey(sm2K.privKey,nil)
	_, err = ki.KeyImport(raw, &mocks2.KeyImportOpts{})
	require.NoError(t, err)
}
...
func TestSM2PrivateKeyImportOptsKeyImporter(t *testing.T) {
	...
	k, err := sm2.GenerateKey(rand.Reader)
	require.NoError(t, err)
	raw,err:=gmX509.MarshalPKISM2PrivateKey(k,nil)
	_, err = ki.KeyImport(raw, &mocks2.KeyImportOpts{})
	require.NoError(t, err)
...
func TestX509PublicKeyImportOptsKeyImporter(t *testing.T) {
	...
	_, err := ki.KeyImport("Hello World", &mocks2.KeyImportOpts{})
	require.Error(t, err)
	require.Contains(t, err.Error(), "Invalid raw material. Expected *gmX509.Certificate.")
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "Certificate's public key type not recognized. Supported keys: [SM2]")
}
