...
import (
	"crypto/elliptic"
	"crypto/sha256"
	gmbccsp "github.com/Hyperledger-TWGC/fabric-plugins/tjfoc-gm-plugins/bccsp"
	"github.com/Hyperledger-TWGC/fabric-plugins/tjfoc-gm-plugins/bccsp/sw"
	"github.com/Hyperledger-TWGC/fabric-plugins/tjfoc-gm-plugins/bccsp/utils"
...
func (csp *Provider) KeyGen(opts bccsp.KeyGenOpts) (k bccsp.Key, err error) {
	...
	//case *gmbccsp.SM2P256KeyGenOpts:
...
func (csp *Provider) Sign(k bccsp.Key, digest []byte, opts bccsp.SignerOpts) ([]byte, error) {
	...
	switch key := k.(type) {
	case *sm2PrivateKey:
		return csp.signSM2(*key, digest)
	default:
		return csp.BCCSP.Sign(key, digest, opts)
	}
}

func (csp *Provider) signSM2(k sm2PrivateKey, digest []byte) ([]byte, error) {
	r, s, err := csp.signP11SM2(k.ski, digest)
	if err != nil {
		return nil, err
	}

	s, err = utils.SM2ToLowS(k.pub.pub, s)
	if err != nil {
		return nil, err
	}

	return utils.MarshalSM2Signature(r, s)
}
...
func (csp *Provider) Verify(k bccsp.Key, signature, digest []byte, opts bccsp.SignerOpts) (bool, error) {
	...
	switch key := k.(type) {
	case *sm2PrivateKey:
		return csp.verifySM2(key.pub, signature, digest)
	case *sm2PublicKey:
		return csp.verifySM2(*key, signature, digest)
	default:
...
func (csp *Provider) verifySM2(k sm2PublicKey, signature, digest []byte) (bool, error) {
	r, s, err := utils.UnmarshalSM2Signature(signature)
	if err != nil {
		return false, fmt.Errorf("Failed unmashalling signature [%s]", err)
	}

	lowS, err := utils.SM2IsLowS(k.pub, s)
	if err != nil {
		return false, err
	}
	if !lowS {
		return false, fmt.Errorf("Invalid S. Must be smaller than half the order [%s][%s]", s, utils.GetCurveHalfOrdersAtSM2(k.pub.Curve))
	}

	if csp.softVerify {
		sm2Sign,err:=utils.MarshalSM2Signature(r,s)
		if err != nil{
			return false,err
		}
		return  k.pub.Verify(digest,sm2Sign),nil
...
func (csp *Provider) getECKey(ski []byte) (pubKey *sm2.PublicKey, isPriv bool, err error) {
...
func (csp *Provider) generateECKey(curve asn1.ObjectIdentifier, ephemeral bool) (ski []byte, pubKey *sm2.PublicKey, err error) {
	session, err := csp.getSession()
	if err != nil {
		return nil, nil, err
	}
	...
	pubGoKey := &sm2.PublicKey{Curve: nistCurve, X: x, Y: y}
...
func (csp *Provider) signP11SM2(ski []byte, msg []byte) (R, S *big.Int, err error) {
	session, err := csp.getSession()
	if err != nil {
		return nil, nil, err
	}
...
func (csp *Provider) verifyP11SM2(ski []byte, msg []byte, R, S *big.Int, byteSize int) (bool, error) {
	session, err := csp.getSession()
	if err != nil {
		return false, err
	}
