...
import (
	...
	"github.com/Hyperledger-TWGC/fabric-plugins/tjfoc-gm-plugins/bccsp/mocks"
	mocks2 "github.com/Hyperledger-TWGC/fabric-plugins/tjfoc-gm-plugins/bccsp/sw/mocks"
...
func TestKeyGenInvalidInputs(t *testing.T) {
	...
	_, err = csp.KeyGen(&gmbccsp.SM2KeyGenOpts{})
...
func TestKeyDerivInvalidInputs(t *testing.T) {
	...
	_, err = csp.KeyDeriv(nil, &gmbccsp.SM2ReRandKeyOpts{})
	require.Error(t, err)
	...
	_, err = csp.KeyDeriv(&mocks.MockKey{}, &gmbccsp.SM2ReRandKeyOpts{})
	require.Error(t, err)
...
func TestKeyImportInvalidInputs(t *testing.T) {
	...
	_, err = csp.KeyImport(nil, &gmbccsp.SM4ImportKeyOpts{})
	require.Error(t, err)
...
func TestEncryptInvalidInputs(t *testing.T) {
	...
	_, err = csp.Encrypt(nil, []byte{1, 2, 3, 4}, &gmbccsp.SM4CBCPKCS7ModeOpts{})
	require.Error(t, err)
	require.Contains(t, err.Error(), "Invalid Key. It must not be nil.")

	_, err = csp.Encrypt(&mocks.MockKey{}, []byte{1, 2, 3, 4}, &gmbccsp.SM4CBCPKCS7ModeOpts{})
	require.Error(t, err)
...
func TestDecryptInvalidInputs(t *testing.T) {
	...
	_, err = csp.Decrypt(nil, []byte{1, 2, 3, 4}, &gmbccsp.SM4CBCPKCS7ModeOpts{})
	require.Error(t, err)
	require.Contains(t, err.Error(), "Invalid Key. It must not be nil.")

	_, err = csp.Decrypt(&mocks.MockKey{}, []byte{1, 2, 3, 4}, &gmbccsp.SM4CBCPKCS7ModeOpts{})
	require.Error(t, err)
