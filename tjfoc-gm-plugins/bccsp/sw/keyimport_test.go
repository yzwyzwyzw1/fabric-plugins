...
import (
	...
	stdx509 "crypto/x509"
	"errors"
	"github.com/Hyperledger-TWGC/fabric-plugins/tjfoc-gm-plugins/bccsp/sw/mocks"
	mocks2 "github.com/Hyperledger-TWGC/fabric-plugins/tjfoc-gm-plugins/bccsp/mocks"
...
func TestSM2PKIXPublicKeyImportOptsKeyImporter(t *testing.T) {
	...
	raw,err:=gmX509.MarshalSm2PublicKey(&k.PublicKey)
	require.NoError(t, err)
...
func TestSM2KeyGenAndImport(t *testing.T){
	...
	raw,err:=gmX509.MarshalSm2PrivateKey(sm2K.privKey,nil)
	_, err = ki.KeyImport(raw, &mocks2.KeyImportOpts{})
	require.NoError(t, err)
}
...
func TestSM2PrivateKeyImportOptsKeyImporter(t *testing.T) {
	...
	k, err := sm2.GenerateKey(rand.Reader)
	require.NoError(t, err)
	raw,err:=gmX509.MarshalSm2PrivateKey(k,nil)
	_, err = ki.KeyImport(raw, &mocks2.KeyImportOpts{})
	require.NoError(t, err)
