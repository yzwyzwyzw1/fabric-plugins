...
func TestVerifySM2(t *testing.T) {
	...
	S.Add(utils.GetCurveHalfOrdersAtSM2(sm2.P256Sm2()), big.NewInt(1))
...
func TestSM2PrivateKey(t *testing.T) {
	...
	ski = k.SKI()
	raw,_:=gmX509.MarshalSm2PublicKey(&k.privKey.PublicKey)
...
func TestSM2PublicKey(t *testing.T) {
	...
	ski = k.SKI()
	raw,_ := gmX509.MarshalSm2PublicKey(k.pubKey)
	hash := sm3.New()
	hash.Write(raw)
	ski2 := hash.Sum(nil)
	...
	bytes, err := k.Bytes()
	require.NoError(t, err)
	bytes2,err := gmX509.MarshalSm2PublicKey(k.pubKey)
...
func TestSm2GenerateKey(t *testing.T) {
	...
	lowLevelPrivateKey, err := sm2.GenerateKey(rand.Reader)
	require.NoError(t, err)
	log.Println("lowLevelPrivateKey:",lowLevelPrivateKey)
